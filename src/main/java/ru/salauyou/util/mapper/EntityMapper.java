...
public abstract class EntityMapper<S, D> implements Mapper<S, D> {
  ...
  final Map<String, List<Class<?>>> currentTypes = new HashMap<>();
  ...
  public final void init() {
    ...
    for (Method m : actual.getMethods()) {
      ...
      if (mapTo != null) {
        ...
        if (ai != null && ai.value().length > 0) {
          for (Class<?> cl : ai.value()) {
            currentTypes.computeIfAbsent(currentPrefix, p -> new ArrayList<>()).add(cl);
          }
        }
        try {
  ...
  public final class Mapping<T> {
    ...
    public <R> Mapping<R> from(Mapper<? super S, ? extends R> mapper) {
      ...
      if (i.hasNext()) {
        ...
        if (e.getMethodName().equals("map") && e.getClassName().contains("EntityMapper")) {
          e = i.next();
        }
